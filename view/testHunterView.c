...
int main(void)
{
	...
	{///////////////////////////////////////////////////////////////////
	...
	
		{
			...
			printf("pathLength: %d\n", pathLength);
			printf("MyCalc  %d: %d, %d, %d, %d\n"
				   "AnsGiv  %d: %d, %d, %d, %d\n", 
					MARSEILLES, path[0], path[1], path[2], path[3], 
					MARSEILLES, GENOA, VENICE, BUDAPEST, CONSTANTA);
		...
		
		{
			...
			printf("pathLength: %d\n", pathLength);
			printf("MyCalc  %d: %d, %d, %d, %d, %d, %d, %d\n"
				   "AnsGiv  %d: %d, %d, %d, %d, %d, %d, %d\n", 
					LISBON, path[0], path[1], path[2], path[3], path[4], path[5], path[6], 
					LISBON, SARAGOSSA, MARSEILLES, GENOA, VENICE, BUDAPEST, GALATZ, CASTLE_DRACULA);
	...
	{///////////////////////////////////////////////////////////////////
		...
		printf("Checking where dracula can go by type "
		       "(Dracula)\n");
	...
	{///////////////////////////////////////////////////////////////////
		...
		printf("Testing Dracula's last location -- TP\n");
	...
	// our own test (real play)
	{///////////////////////////////////////////////////////////////////
	
		printf("Real Play part No.1 (test everything)\n");
		
		char *trail =
        "GMN.... SMA.... HBI.... MAT.... DGA.V.. "
        "GLO.... SSR.... HRO.... MVA.... DC?T... "
        "GEC.... STO.... HFL.... MSO.... DC?T... "
        "GLE.... SMR.... HVE.... MGAV... DSOT... "
        "GBU.... SGO.... HBD.... MKL.... DC?T... "
        "GCO.... SVE.... HSZ.... MBF.... DD1T... "
		"GFR.... SFL.... HZA.... MSOTTD. DC?T...";
		...
		assert(HvGetScore(hv) == GAME_START_SCORE
                                 - 7 * SCORE_LOSS_DRACULA_TURN);
		int round = -1;
		assert(HvGetLastKnownDraculaLocation(hv, &round) == SOFIA);
		assert(round == 3);
		assert(HvGetVampireLocation(hv) == NOWHERE);
	...
	{///////////////////////////////////////////////////////////////////
		...
		printf("Real Play part No.2 (test everything)\n");
        ...
        "GLE.... SMR.... HVE.... MGAV... DSOT... "
        "GBU.... SGO.... HBD.... MKL.... DC?T... "
        "GCO.... SVE.... HSZ.... MBF.... DD2T... "
		"GFR.... SFL.... HZA.... MSOTTD. DC?T... "
		"GNU.... STS.... HSJ.... MSA.... DC?T.M. "
		"GMU.... SIO.... HVAT... MVA.... DS?..M. "
		"GZA.... STS.... HIO.... MIO.... DSAT... "
		"GSJ.... SIO.... HSATD.. MSAD...";
		...
		assert(HvGetScore(hv) == GAME_START_SCORE
                                 - 10 * SCORE_LOSS_DRACULA_TURN
								 - SCORE_LOSS_HUNTER_HOSPITAL);
		int round = -1;
		assert(HvGetLastKnownDraculaLocation(hv, &round) == SALONICA);
		assert(round == 9);
		assert(HvGetVampireLocation(hv) == NOWHERE);
		assert(HvGetHealth(hv, PLAYER_MINA_HARKER) == 0); // dead);
		assert(HvGetHealth(hv, PLAYER_VAN_HELSING) == GAME_START_HUNTER_LIFE_POINTS
		                                              - 2 * LIFE_LOSS_TRAP_ENCOUNTER
													  - 1 * LIFE_LOSS_DRACULA_ENCOUNTER);
		assert(HvGetHealth(hv, PLAYER_DRACULA) == GAME_START_BLOOD_POINTS 
                                                  - LIFE_LOSS_SEA
                                                  - 3 * LIFE_LOSS_HUNTER_ENCOUNTER);
	...
	{///////////////////////////////////////////////////////////////////
		...
		printf("Real Play part No.3 (test everything)\n");
        ...
        "GCO.... SVE.... HSZ.... MBF.... DD1T... "
		"GFR.... SFL.... HZA.... MSOTTD. DC?T... "
		"GNU.... STS.... HSJ.... MSA.... DC?T... "
		"GMU.... SIO.... HVAT... MVA.... DS?.... "
		"GZA.... STS.... HIO.... MIO.... DSAT... "
		"GSJ.... SIO.... HSATD.. MSAD... DSOT... "
		"GSOTD..";
		...
		assert(HvGetVampireLocation(hv) == NOWHERE);
		assert(HvGetHealth(hv, PLAYER_LORD_GODALMING) == GAME_START_HUNTER_LIFE_POINTS
		                                              - 1 * LIFE_LOSS_TRAP_ENCOUNTER
                                                      - 1 * LIFE_LOSS_DRACULA_ENCOUNTER);
		assert(HvGetHealth(hv, PLAYER_MINA_HARKER) == 0); // not revived yet);
		assert(HvGetPlayerLocation(hv, PLAYER_MINA_HARKER) == HOSPITAL_PLACE); // once dead
		assert(HvGetHealth(hv, PLAYER_VAN_HELSING) == GAME_START_HUNTER_LIFE_POINTS
		                                              - 2 * LIFE_LOSS_TRAP_ENCOUNTER
                                                      - 1 * LIFE_LOSS_DRACULA_ENCOUNTER);
		assert(HvGetHealth(hv, PLAYER_DRACULA) == 0); //dead
